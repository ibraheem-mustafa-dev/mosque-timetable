...
*Created: 2026-02-24 | Last updated: 2026-03-01*
*Status: ACTIVE — UI/UX review complete, Phase 0 done, Phase 0.5 starting*
...
### Grade Ladder
| Grade | What it means | What's needed |
|-------|--------------|---------------|
| **C+** | Functional but visually generic, UX broken in places | Where we are now |
| **B** | Fixed critical UX bugs, clean Islamic design identity | Phase 0.5 complete |
| **A-** | Full admin UX overhaul, setup wizard, WP.org submission | Phase 1 complete |
| **A** | On WP.org, 100+ installs, Pro tier live | Phase 2 complete |
| **S** | Directory network effect, WhatsApp viral, 1,000+ installs | Phase 3 complete |

**The honest reason we're C+ not B-:** Code quality is B-, but code quality isn't what users see. The product *experience* is C+. The design has zero Islamic identity, PWA install banner covers the prayer data, Today's Prayers is a 404, and there's no setup wizard. We fixed the engineering. Now we fix the product.

**What separates each tier:**
- **B:** Looks like a mosque plugin. Feels intentional. Critical bugs fixed.
...
**Goal:** Working plugin, code quality baseline, ready to submit

1. ✅ Fix admin AJAX
2. ✅ Split monolithic 10,717-line file into 3 files (mosque-timetable.php, helpers.php, class-mosque-timetable.php)
3. ✅ PHPCS: 730 → 146 violations (all remaining are docs-only, zero functional issues)
4. ✅ Created readme.txt + uninstall.php for WP.org
5. ✅ Version bumped to 3.1.0
6. ✅ Deployed to mosquewebdesign.com
7. ⏳ Screenshots still needed (after Phase 0.5 redesign — take them THEN, not now)
8. ⏳ WP.org submission (after Phase 0.5 — submit with good-looking screenshots)

**Deliverable:** Code is WP.org-ready. Design is not yet. Submit after Phase 0.5.

---

### Phase 0.5: UI/UX Redesign ← CURRENT PHASE
**Goal:** The plugin looks and feels like it was built for mosques, not generated by AI
**Grade jump:** C+ → B

**The problem in one sentence:** Purple-to-blue gradient, emoji icons, zero Islamic visual identity, and a PWA banner that covers the actual prayer data.

#### Critical Bug Fixes (do first, 2-4 hours)
1. Fix /todays-prayers/ 404 — flush rewrite rules or recreate the WP page
2. Redesign PWA install banner — dismissible toast at bottom, NOT a blocking overlay
3. Remove duplicate month selectors on prayer times page (one at top, one below header)
4. Fix "Unable to calculate next prayer time" error — add fallback and guidance message
5. Remove the unrendered `[wp_social_ninja]` shortcode on demo site

#### The Islamic Design System
Replace the generic gradient palette with a design language rooted in Islamic architecture:

**Colour palette:**
```css
--mosque-primary:     #0D7377; /* Deep Teal (mosque domes, Masjid al-Nabawi) */
--mosque-secondary:   #C5A55A; /* Warm Gold (Islamic calligraphy, gilded tiles) */
--mosque-accent:      #1A3A5C; /* Midnight Blue (night sky, Ramadan mood) */
--mosque-bg:          #F5F1EB; /* Warm Cream (old parchment, Islamic manuscripts) */
--mosque-surface:     #FFFFFF;
--mosque-text:        #1A2332; /* Near black, warm not cold */
--mosque-text-muted:  #5A6978;
--mosque-friday:      rgba(197, 165, 90, 0.12); /* Gold tint for Jummah rows */
--mosque-today:       rgba(13, 115, 119, 0.10); /* Teal tint for today's row */
--mosque-next-prayer: rgba(13, 115, 119, 0.20); /* Stronger teal for next prayer */
```

**Typography (Google Fonts):**
- **Headings:** El Messiri 600/700 — Arabic-inspired geometric sans. Feels Islamic, remains readable.
- **Body:** DM Sans 400/500 — Clean, modern, excellent readability at small sizes.
- **Prayer times display:** Space Mono 400 — Monospaced so times align perfectly in columns.
- **Arabic text (Hijri dates):** Amiri — Beautiful Naskh-style, suitable for Quranic/Islamic references.

**Visual elements:**
- Subtle SVG Islamic geometric pattern as header background (8-pointed star tiling, low opacity)
- Custom SVG icon set: crescent moon, minaret, prayer mat, compass, clock — no emoji
- Crescent motif in the plugin logo/branding
- Gradient replaced with flat teal header + gold accent bar

#### Frontend Redesign (8-12 hours)
- **Today's Prayer Card** — Hero component, current prayer large, live countdown, next prayer shown below, one-tap to reveal jamaat times, WhatsApp share button
- **Monthly Timetable** — Sticky header on scroll, today row highlighted teal, Fridays gold-tinted, clean column hierarchy (prayer name > start time > jamaat time)
- **Mobile** — Cards already work, but fix Fajr labelling, make jamaat times equally prominent (that's what people actually need), fix header showing mosque address twice
- **Navigation** — Replace "View Archive | Today's Prayers | Home" with proper nav links
- **Remove button soup** — Six action buttons is too many. Primary: "Download PDF". Secondary: "Subscribe to Calendar". Tertiary: "Export ICS". Everything else behind a "More options" link.

#### Admin Panel Redesign (6-8 hours)
- Replace purple gradient header with teal + Islamic geometric pattern
- Add a **Dashboard overview** page (today's times, current month data status, quick actions)
- Improve time input UX — HTML5 `<input type="time">` where possible, inline validation
- Make Hijri dates column auto-populate when dates are generated (not all blank)
- Clarify the clock icon buttons — add tooltips
- Improve Appearance Settings — add font selector, add 3 preset themes (Classic, Modern Dark, Minimal White)

#### Deliverables
- Updated mosque-timetable.css (Islamic design system)
- Updated mosque-timetable-admin.css
- El Messiri + DM Sans + Space Mono loaded from Google Fonts (with local fallback)
- SVG icon set committed to /assets/icons/
- Islamic geometric pattern SVG in /assets/
- All 5 WP.org screenshots taken after redesign
- Deploy to mosquewebdesign.com
...
### Month 1: Fix & Redesign (Phase 0.5) ← NOW
**Grade target: C+ → B**

**Week 1 — Critical bugs (do immediately):**
- [ ] Fix /todays-prayers/ 404
- [ ] Redesign PWA banner (non-blocking toast)
- [ ] Remove duplicate month selectors
- [ ] Fix "Unable to calculate next prayer time" error
- [ ] Remove unrendered shortcode from demo

**Week 2-3 — Frontend CSS redesign:**
- [ ] Implement Islamic design system (teal/gold/cream palette)
- [ ] Add El Messiri + DM Sans + Space Mono fonts
- [ ] Create SVG icon set (crescent, minaret, prayer mat, compass)
- [ ] Add Islamic geometric pattern SVG background
- [ ] Redesign Today's Prayer Card as hero component
- [ ] Redesign monthly timetable (sticky header, proper row highlighting)
- [ ] Fix mobile card view (Fajr label, jamaat prominence)
- [ ] Reduce button soup to 3 primary actions

**Week 4 — Admin redesign + screenshots:**
- [ ] Replace admin header with Islamic design
- [ ] Add dashboard overview page
- [ ] Improve time inputs (HTML5 type="time")
- [ ] Add 3 preset themes (Classic, Modern Dark, Minimal White)
- [ ] Take 5 proper WP.org screenshots on redesigned demo
- [ ] Submit to WordPress.org

### Month 2: First Users (Phase 1 starts)
**Grade target: B → A-**

- [ ] Build UK mosque WordPress list (300-500 targets)
- [ ] Start direct email outreach (50 mosques)
- [ ] Ship setup wizard (3-step: mosque name/location > calc method > generate year)
- [ ] Ship Ramadan mode (Suhoor/Iftar/Taraweeh — timing is NOW, Ramadan 2026 is live)
- [ ] Ship WhatsApp share button
- [ ] Ask installed mosques for WP.org reviews

### Month 3: Growth (Phase 1 completes)
**Grade target: A- → A**

- [ ] Ship digital screen mode (mosque TV/display)
- [ ] Ship Gutenberg block
- [ ] Ship Jummah khutbah schedule
- [ ] Product Hunt launch (after 10+ WP.org reviews)
- [ ] First YouTube tutorial ("How to display prayer times on your mosque website")
- [ ] First Facebook group posts (tutorial format, not promotional)
- [ ] Begin directory API backend
- [ ] Ship One-Click Aladhan Import with map selector

### Month 4: Monetise (Phase 2 starts)
- [ ] Launch Pro tier (£49/year)
- [ ] Ship push notifications (Pro feature)
- [ ] Ship announcement system
- [ ] Ship event calendar
- [ ] Begin directory beta (opt-in for installed mosques)
- [ ] Second round of mosque outreach (100 more)

### Month 5-6: Network (Phase 2 completes, Phase 3 starts)
**Grade target: A → S**

- [ ] Launch Organisation tier (£199/year)
- [ ] Ship multi-mosque dashboard
- [ ] Ship analytics dashboard
- [ ] Directory live with 50+ mosques
- [ ] Launch mosquefinder.com (or dedicated subdomain)
- [ ] API tier for developers (£29/month)
- [ ] First conference appearance
- [ ] Ship dark mode

### Grade Checkpoints
| Checkpoint | Grade | Trigger |
|-----------|-------|---------|
| Phase 0.5 complete | B | Islamic design live, critical bugs fixed, WP.org submitted |
| Phase 1 complete | A- | Setup wizard, Ramadan mode, WhatsApp share, 20+ installs |
| Phase 2 complete | A | On WP.org, 100+ installs, Pro tier live, reviews coming in |
| Phase 3 complete | S | Directory with 50+ mosques, WhatsApp viral loop running, 1,000+ installs |

---

## GRADE: S (target) | C+ (current)

**Starting point reality check (added 2026-03-01 after full UI/UX audit):**
The code is B- but the product experience is C+. This is fine — code quality was the hard part and it's done. The design and UX gaps are solvable in 2-3 weeks of focused work. The foundation is solid; the face needs replacing.

**Why S and not S+:**
- S+ requires the mobile app and voice assistant integration (6-12 month projects)
...
- Ramadan mode needs to ship FAST — Ramadan 2026 is already live (1 March)

**Website Template timing:**
Build this AFTER WP.org has 10+ reviews. Package as "Mosque Website Kit" — Astra child theme + Plugin Pro pre-configured = £149/year bundle. Not before.
